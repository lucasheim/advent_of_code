...
function findMostSleptMinuteForLaziestGuard(registers) {
  ...
  let mostAsleepGuard = "";
  let maxAsleepMinutes = 0;

  for (let guard of Object.keys(guardRegisters)) {
    const asleepMinutes = Object.keys(guardRegisters[guard])
                                              .reduce((acc, minute) => acc + guardRegisters[guard][minute], 0);
    if (asleepMinutes > maxAsleepMinutes) {
      maxAsleepMinutes = asleepMinutes;
  ...
  let maxTimesAsleepMinute = 0;
  for (let minute of Object.keys(guardRegisters[mostAsleepGuard])) {
    if (guardRegisters[mostAsleepGuard][minute] > maxTimesAsleepMinute) {
      maxTimesAsleepMinute = guardRegisters[mostAsleepGuard][minute];
      mostAsleepMinute = minute;
    }
  }

  return mostAsleepMinute * mostAsleepGuard;
}

AnswerPrinter.printAnswerWithTime(findMostSleptMinuteForLaziestGuard, input.split("\n"));